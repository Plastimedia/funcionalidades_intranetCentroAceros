...
export default function Edit({ post, roles }: { post: any, roles: any[] }) {
    const { data, setData, post: updatePost, processing, errors } = useForm({
        _method: 'PUT', // Needed for file uploads in Laravel PUT requests
        title: post.title,
    ...
    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        updatePost(route('admin.posts.update', post.id));
    };
            ...
            <div className="mx-auto max-w-4xl rounded-3xl bg-white p-8 shadow-sm ring-1 ring-slate-100">
                <form onSubmit={submit} className="space-y-6" encType="multipart/form-data">
                    ...
                    <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                        <div>
                            <InputLabel htmlFor="image" value="Imagen Principal (Reemplazar)" />
                            {post.image_path && (
                                <div className="mb-2">
                                    <img src={`/storage/${post.image_path}`} alt="Actual" className="h-20 w-32 rounded-lg object-cover" />
                                </div>
                            )}
                            <input
                                type="file"
